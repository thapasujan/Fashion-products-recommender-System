...
def recommend_images(request):
    ...
    if request.method == 'POST' and form.is_valid():
        ...
        distances, indices = neighbors.kneighbors([input_img_features])
         ...
         # Introduce randomness into recommendations
        neighbor_indices = indices[0][1:]  # Exclude the first one (itself)
        random.shuffle(neighbor_indices)  # Shuffle indices for randomness
        diverse_indices = neighbor_indices[:5]  # Select the top 5 random neighbors
        
        # Collect recommended image URLs
        for idx in diverse_indices:  # Skip the first one (itself)
